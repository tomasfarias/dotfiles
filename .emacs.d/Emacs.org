...
** Git interaction
We use Magit to interact with git in Emacs.
...
#+BEGIN_SRC emacs-lisp :results output silent
  (use-package elfeed
    ...
    (let* ((host "https://github.com/tomasfarias.private.atom")
           ;; Fetches token from auth-source and constructs feed URL.
    ...
    (add-hook 'elfeed-new-entry-hook
              (elfeed-make-tagger :before "2 weeks ago"
                                  :remove 'unread)))
  (global-set-key (kbd "C-c r d") 'elfeed)
#+END_SRC
...
* Language modes
 ...
 #+BEGIN_SRC emacs-lisp :results output silentp
   (use-package pyvenv
     :ensure t
     :init
